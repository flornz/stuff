...
namespace GeoCode
{
    public static class Distance
    {
        ...
        public static double? BetweenTwoLocations(string l1, string l2, Units units)
        {
            var ll1 = addressToLongLat(l1);
            if (!ll1.HasValue) return null;
            var ll2 = addressToLongLat(l2);
        ...
        // Will return a null if the Google API is unable to find either post code, or the country constraint fails
        public static double? BetweenTwoLocations(Coords ll1, string l2, Units units)
        {
            var ll2 = addressToLongLat(l2);
            if (!ll2.HasValue) return null;
            return ll1.DistanceTo(ll2.Value, units);
        }
        ...
        public static Coords? addressToLongLat(string address)
        {
            var client = new WebClient();
            var encodedAaddress = HttpUtility.UrlEncode(address);
            var url = string.Format("https://maps.googleapis.com/maps/api/geocode/json?address={0}", encodedAaddress);
            var stringFullOfJson = client.DownloadString(url);
            ...
            double longitude;
            double lattitude;
            if (!Double.TryParse(location["lat"].ToString(), out longitude)) return null;
            if (!Double.TryParse(location["lng"].ToString(), out lattitude)) return null;
